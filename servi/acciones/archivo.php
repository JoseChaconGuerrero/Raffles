...
if (isset($_FILES["namearchivo"]) && !empty($_FILES["namearchivo"]["name"])) {
  ...
  if (!file_exists($target_path)) {
    if (move_uploaded_file($source, $target_path)) {
      $sqlInsertImg = ("INSERT INTO msjs(user,user_id,to_user,to_id,fecha,nombre_equipo_user,leido, imagen, sonido) VALUES('$user','$user_id','$to_user','$to_id','$FechaMsj','$nombre_equipo_user','$leido','$nuevoNameFoto', 'NO')");
...
if(isset($_FILES["document_upload"]) && !empty($_FILES["document_upload"]["name"])){
  ...
  $source = $_FILES["document_upload"]["tmp_name"];
  ...
  if (!file_exists($target_path)) {
    if (move_uploaded_file($source, $target_path)) {
      $sqlInsertImg = ("INSERT INTO msjs(user,user_id,to_user,to_id,fecha,nombre_equipo_user,leido, archivos, sonido) VALUES('$user','$user_id','$to_user','$to_id','$FechaMsj','$nombre_equipo_user','$leido','$nuevoNameFoto', 'NO')");
      $resulInsertImg = mysqli_query($con, $sqlInsertImg);
    } else {
      echo "Ha ocurrido un error al cargar la imagen, por favor inténtelo de nuevo.<br>";
    }
