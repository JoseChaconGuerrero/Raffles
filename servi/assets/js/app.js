...
fetch('../acciones/getnumbers.php', {
  ...
  .then(data => {
    ...
    const cargarProductos = () => {
      ...
      arreglo.forEach((producto) => {
        ...
        if (producto.status == "deactivate") {
          ...
          fila.addEventListener("click", () => seleccionarFila(fila, producto.number));
        }
    ...
    const cargarItemPaginacion = () => {
      ...
      let inicio = Math.max(2, paginaActiva - Math.floor(maxPaginasVisibles / 2));
    ...
    const agregarItemPaginacion = (index) => {
      ...
      const enlace = `<button class="page-link" onclick="pasarPagina(${index - 1})">${index}</button>`;
